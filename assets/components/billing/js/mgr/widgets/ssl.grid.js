Billing.grid.SSL = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'billing-grid-ssl';
	}
	Ext.applyIf(config, {
...
//			}
	...
	Billing.grid.SSL.superclass.constructor.call(this, config);
...
Ext.extend(Billing.grid.SSL, MODx.grid.Grid, {
	windows: {},
	...
	createSsl: function (btn, e) {
		var w = MODx.load({
			xtype: 'billing-cert-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'billing-cert-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeSsl: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('billing_ssl_remove')
				: _('billing_cert_remove'),
			text: ids.length > 1
				? _('billing_ssl_remove_confirm')
				: _('billing_cert_remove_confirm'),
			url: this.config.url,
			params: {
	...
	getColumns: function (config) {
		return [{
            ...
            header: _('billing.options') +' options'
            ,dataIndex: 'options'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
        },{
            header: _('billing.comments')
            ,dataIndex: 'comments'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
            ...
            header: _('billing.price1')
            ,dataIndex: 'price1'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
        },{
            header: _('billing.price2')
            ,dataIndex: 'price2'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
        },{
            header: _('billing.price3')
            ,dataIndex: 'price3'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
        },{
            header: _('billing.price4')
            ,dataIndex: 'price4'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
        },{
            header: _('billing.price5')
            ,dataIndex: 'price5'
            ,sortable: false
            ,width: 50
            ,editor: { xtype: 'textfield' }
...
Ext.reg('billing-grid-ssl', Billing.grid.SSL);