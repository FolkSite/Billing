Billing.grid.Licens = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'billing-grid-licens';
	}
	Ext.applyIf(config, {
...
//			}
	...
	Billing.grid.Licens.superclass.constructor.call(this, config);
...
Ext.extend(Billing.grid.Licens, MODx.grid.Grid, {
	windows: {},
	...
	createLicens: function (btn, e) {
		var w = MODx.load({
			xtype: 'billing-lic-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'billing-lic-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeLicens: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('billing_licens_remove')
				: _('billing_lic_remove'),
			text: ids.length > 1
				? _('billing_licens_remove_confirm')
				: _('billing_lic_remove_confirm'),
			url: this.config.url,
			params: {
    ...
    ,renderVid: function(val,cell,row) {
        switch (Number(val)) {
            case 1:
                var out = _('billing.lic.ISPmanager');
                break
            case 2:
                var out = _('billing.lic.BILLmanager');
                break
            case 3:
                var out = _('billing.lic.VMmanager');
                break
            case 4:
                var out = _('billing.lic.VEmanager');
                break
            case 5:
                var out = _('billing.lic.DNSmanager');
                break
            case 6:
                var out = _('billing.lic.IPmanager');
                break
            case 7:
                var out = _('billing.lic.cPanel');
                break
            case 8:
                var out = _('billing.lic.Parallels');
                break
            default:
...
Ext.reg('billing-grid-licens', Billing.grid.Licens);