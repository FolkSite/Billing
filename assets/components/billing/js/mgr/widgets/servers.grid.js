Billing.grid.Servers = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'billing-grid-servers';
	}
	Ext.applyIf(config, {
...
//			}
	...
	Billing.grid.Servers.superclass.constructor.call(this, config);
...
Ext.extend(Billing.grid.Servers, MODx.grid.Grid, {
	windows: {},
	...
	createServer: function (btn, e) {
		var w = MODx.load({
			xtype: 'billing-server-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'billing-server-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeServer: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('billing_servers_remove')
				: _('billing_server_remove'),
			text: ids.length > 1
				? _('billing_servers_remove_confirm')
				: _('billing_server_remove_confirm'),
			url: this.config.url,
			params: {
	...
	getColumns: function (config) {
		return [{
            ...
            header: _('billing.virtualisation')
...
Ext.reg('billing-grid-servers', Billing.grid.Servers);